...
const loginUser = async (payload: TLoginUSer) => {
  ...
  const accessToken= createToken(jwtPayload,config.jwt_acess_secret as string,config.jwt_acess_expieres as string)

  const refreshToken= createToken(jwtPayload,config.jwt_refresh_secret as string,config.jwt_referesh_expiers as string)
...
const refreshToken = async (token: string) => {
    ...
    if (
      ...
      userModel.isJWTIssuedBeforePasswordChange(user.passwordChangeAt.type, iat as number)
    ) {
